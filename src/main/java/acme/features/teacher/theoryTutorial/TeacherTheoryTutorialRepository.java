...
public interface TeacherTheoryTutorialRepository extends AbstractRepository {
	...
	@Query("select t from Teacher te, Course c, Register r, TheoryTutorial t where te.id = c.teacher.id and c.id = r.course.id and r.theoryTutorial.id = t.id and te.id = :id and c.id = :masterId")
	Collection<TheoryTutorial> findManyTheoryTutorialsByTeacherAndCourseId(int id, int masterId);
